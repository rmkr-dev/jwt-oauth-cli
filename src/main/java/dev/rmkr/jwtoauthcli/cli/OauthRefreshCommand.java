...
@Command(
    name = "refresh",
    description = "Refresh tokens using a refresh_token grant (POSTs to the token endpoint)")
public class OauthRefreshCommand implements Callable<Integer> {

  @Option(names = "--token-endpoint", required = true, description = "Token endpoint URL")
  String tokenEndpoint;

  @Option(names = "--refresh-token", required = true, description = "Refresh token")
  String refreshToken;
  ...
  public Integer call() {
    ...
    try {
      ...
      Map<String, Object> result = OauthSupport.refreshToken(params);
